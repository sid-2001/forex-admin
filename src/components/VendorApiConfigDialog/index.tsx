...
export default function VendorApiConfigDialog({ open, onClose, editData, refreshList, showAlert, vendors, urlTypes }: any) {
  ...
  useEffect(() => {
    ...
    setErrors({})
  }, [editData, open, vendors, urlTypes])
  ...
  const handleSubmit = async () => {
    ...
    // Create a clean payload (removing the UI-only 'vendor' and 'urlType' objects if your API doesn't want them)
    const { vendor, urlType, ...cleanForm } = form
    const payload = {
      ...cleanForm,
