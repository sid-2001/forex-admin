...
export default class CountryLabelCodesService {
  ...
  async update(payload: CountryLabelCode,id:any): Promise<{
    status: boolean;
    message: string;
    data?: any;
  }> {
  ...
  async getCombinedOptions(
    countryBusinessPayoutPartners: CountryBusinessPayoutPartner[]
  ): Promise<CombinedCountryLabelOption[]> {
    try {
      ...
      return countryLabelCodes.map(code => {
        ...
        const businessInfo = matchingPartner 
          ? ` (${matchingPartner.businessTypeCode}/${matchingPartner.payoutPartner})`
          : '';
