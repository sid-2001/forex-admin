...
const TransactionDashboard = () => {
  ...
  const filteredTransactions = transactions.filter((item) => {
    ...
    // Status Filter - FIXED to match actual status values
    const status = item?.transactionGatewayDTO?.transactionStatus || ''
    let matchesStatus = true
    if (statusFilter === 'SUCCESS') {
      matchesStatus = ['SUCCESS', 'COMPLETED', 'ACCEPTED', 'EXECUTED'].includes(status)
    } else if (statusFilter === 'PENDING') {
      matchesStatus = ['PENDING', 'IN_PROGRESS', 'PROCESSING', 'INITIATED'].includes(status)
    } else if (statusFilter === 'FAILED') {
      matchesStatus = ['FAILED', 'REJECTED', 'CANCELLED', 'DECLINED'].includes(status)
    }
    // 'ALL' - matchesStatus remains true
  ...
  const successfulTransactions = filteredTransactions.filter((item) => {
    const status = item?.transactionGatewayDTO?.transactionStatus || ''
    return ['SUCCESS', 'COMPLETED', 'ACCEPTED', 'EXECUTED'].includes(status)
  }).length

  const pendingTransactions = filteredTransactions.filter((item) => {
    const status = item?.transactionGatewayDTO?.transactionStatus || ''
    return ['PENDING', 'IN_PROGRESS', 'PROCESSING', 'INITIATED'].includes(status)
  }).length
  ...
  const amountDistributionOptions: any = {
    subtitle: { text: 'Amount Range Histogram' },
    ...
    axes: [
      {
        type: 'category',
        position: 'bottom',
        title: { text: `Amount Range (${selectedCountryCurrency})` },
      },
      {
        type: 'number',
        position: 'left',
  ...
  const trendOptions: any = {
    title: { enabled: false },
    subtitle: { text: 'Daily Transactions' },
    data: trendData.length > 0 ? trendData : [{ date: 'No Data', count: 0 }],
    series: [
      {
        type: 'line',
        xKey: 'date',
    ...
    axes: [
      ...
      { type: 'number', position: 'left', title: { text: 'Transactions' } },
    ],
  }

  // Pie Data - ensure values are positive for display
  const pieData = [
    {
      status: 'Successful',
      value: successfulTransactions > 0 ? successfulTransactions : 0.1,
      percentage: totalTransactions > 0 ? ((successfulTransactions / totalTransactions) * 100).toFixed(1) : 0,
    },
    {
      status: 'Pending',
      value: pendingTransactions > 0 ? pendingTransactions : 0.1,
      percentage: totalTransactions > 0 ? ((pendingTransactions / totalTransactions) * 100).toFixed(1) : 0,
    },
    {
      status: 'Failed',
      value: failedTransactions > 0 ? failedTransactions : 0.1,
  ...
  const topCorridors = Object.values(
    filteredTransactions.reduce((acc: any, item: any) => {
      ...
      if (!acc[key]) {
        acc[key] = { corridor: key, volume: 0, transactions: 0 }
      }
  ...
  const topDestinations = Object.values(
    filteredTransactions.reduce((acc: any, item: any) => {
      ...
      if (!acc[country]) {
        acc[country] = { country, volume: 0, transactions: 0 }
      }
  ...
  // Summary Cards
  const summaryCards = [
    { title: 'Total Transactions', value: totalTransactions, icon: <ReceiptLongIcon fontSize="large" />, color: '#1976d2' },
    { title: 'Successful', value: successfulTransactions, icon: <CheckCircleIcon fontSize="large" />, color: '#2e7d32' },
    { title: 'Pending', value: pendingTransactions, icon: <PendingActionsIcon fontSize="large" />, color: '#ed6c02' },
    { title: 'Failed', value: failedTransactions, icon: <CancelIcon fontSize="large" />, color: '#d32f2f' },
    {
      title: 'Total Value',
      value: `${Number(totalVolume).toLocaleString()} ${selectedCountryCurrency}`,
      icon: <ReceiptLongIcon fontSize="large" />,
      color: '#9c27b0',
    },
    { title: 'Success Rate', value: `${successRate}%`, icon: <CheckCircleIcon fontSize="large" />, color: '#00c853' },
    {
    ...
    { title: 'Peak Day', value: peakDay?.date || '-', icon: <ReceiptLongIcon fontSize="large" />, color: '#ff9800' },
  ]

  const fetchOutwardTransactions = async () => {
    try {
      const response = await transactionService.getOutwardAllTransaction(userCountry, 0, 5000)
      console.log('Transactions fetched:', response?.length || 0)
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            {topCorridors.length > 0 ? (
              ...
              <Typography sx={{ py: 2, textAlign: 'center', color: '#64748B' }}>No corridor data available</Typography>
            )}
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            {topSources.length > 0 ? (
              ...
              <Typography sx={{ py: 2, textAlign: 'center', color: '#64748B' }}>No source country data available</Typography>
            )}
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            {topDestinations.length > 0 ? (
              ...
              <Typography sx={{ py: 2, textAlign: 'center', color: '#64748B' }}>No destination country data available</Typography>
            )}
          </Card>
        </Grid>
      </Grid>
