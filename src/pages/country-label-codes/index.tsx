...
import CountryLabelCodesService, {
  CountryLabelCode,
  CombinedCountryLabelOption
} from "../../services/country-label-codes.service";
import CountryBusinessPayoutPartnerService, {
  CountryBusinessPayoutPartner
} from "../../services/countryBusinessPayoutPartner.service";
...
export default function CountryLabelCodesGridPage() {
  ...
  const filterRailPayoutMappingsByCountry = (countryCode: string) => {
    try {
      ...
      const filtered = allRailPayoutMappings.filter(mapping => {
        ...
        if (mapping.countryCorridorBusinessMapCode && 
            mapping.countryCorridorBusinessMapCode.includes(countryCode)) {
          return true;
        }
  ...
  const filteredRows = rows.filter(row => {
    ...
    const railInfo = railMapping ? 
      `${railMapping.businessTypeCode} ${railMapping.payoutPartner}`.toLowerCase() : '';
    
    // Find BOP category details
    const bopCategory = bopCategories.find(c => c.bopPurposeCategoryCode === row.countryReportingCode);
    const bopInfo = bopCategory ? 
      `${bopCategory.bopCategoryType} ${bopCategory.bopCategoryDescription}`.toLowerCase() : '';
  ...
  return (
    <Box sx={{ height: "100vh", p: 3 }}>
      <Stack direction="row" justifyContent="space-between" mb={2}>
        <Typography variant="h5"><b>Country Label Codes</b></Typography>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center', bgcolor: 'info.light', color: 'white' }}>
            <Typography variant="h6">
              {[...new Set(rows.map(r => r.countryCode))].length}
            </Typography>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center', bgcolor: 'warning.light', color: 'white' }}>
            <Typography variant="h6">
              {[...new Set(rows.map(r => r.channel))].length}
            </Typography>
            ...
            <FormControl fullWidth required>
              ...
              >
                {loadingRailMappings ? (
                  ...
                  <MenuItem disabled>
                    {form.countryCode ? 
                      `No rail payout mappings found for ${getCountryName(form.countryCode)}` : 
              ...
              {form.countryCode && filteredRailPayoutMappings.length === 0 && !loadingRailMappings && (
                <Typography variant="caption" color="textSecondary" sx={{ mt: 1 }}>
                  No active rail payout mappings found for {getCountryName(form.countryCode)}. 
                  Please add mappings in the Country Business Payout Partner section first.
                </Typography>
              )}
            </FormControl>
            ...
            <FormControl fullWidth required>
              ...
              >
                {loadingBopCategories ? (
                  ...
                  <MenuItem disabled>
                    {form.countryCode ? 
                      `No BOP categories found for ${getCountryName(form.countryCode)}` : 
                      "Please select a country first"
                    }
                  </MenuItem>
                ) : (
                  filteredBopCategories
                    .map((category) => (
                      <MenuItem key={category.bopPurposeCategoryCode} value={category.bopPurposeCategoryCode}>
                        <Box>
                          ...
                          <Typography variant="caption" display="block" color={category.effectiveDateValid ? "success" : "error"}>
                            {category.effectiveDateValid ? "Active" : "Inactive"} | 
                            {new Date(category.effectiveFromDate).toLocaleDateString()} - {new Date(category.effectiveToDate).toLocaleDateString()}
                          </Typography>
                        </Box>
                      </MenuItem>
                    ))
              ...
              {form.countryCode && filteredBopCategories.length === 0 && !loadingBopCategories && (
                <Typography variant="caption" color="textSecondary" sx={{ mt: 1 }}>
                  No active BOP categories found for {getCountryName(form.countryCode)}. 
                  Please add BOP categories first.
                </Typography>
              )}
            </FormControl>
